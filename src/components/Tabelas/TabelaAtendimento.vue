    ...
    >
        ...
        <template v-slot:top-left>
            <div class="row items-center">
                <q-btn @click="recebeDadosTabela(tipoTabela)" size="12px" color="green" icon="update" round>
                ...
                setTimeout(() => {
                    Delete('v1/atendimento/' + id).then(res => {
                        if (res.data.status == true) {
                            ...
                            this.recebeDadosTabela(this.TipoTabela)
                            // this.recebeDadosTabela('A')
                            // this.recebeDadosTabela('B')
                            // this.recebeDadosTabela('C')
                        } else {
                            this.$q.notify({
                                message: res.data.msg,
                                color: 'green',
